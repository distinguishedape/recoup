...
### D50. Spike finding F1 was not too strong after all — retracted, see D58
...
*They were paid within the hour, and the answer was not the one this section expected. See
[[D58]].*

### D58. The error cards were paid, and F1 holds. D50 is retracted.

All eight documented error-scenario cards were paid, choosing *Failure* on the mock bank
screen as the documentation instructs. Every one returned:
...
Identical in every field but the id. The cards used were confirmed against the `last4` on each
payment's card entity, so this is not a mistyped run — `...0001` really was the documented
`insufficient_fund` card, and it returned the same generic string as the other seven.

**D50 was wrong and is retracted.** It claimed F1 — *test mode cannot inject decline reasons*
— was too strong, on the grounds that these cards exist and are published. They do exist. On
this account they do not differentiate, and the claim was made from reading documentation
rather than from running it. The lesson is the one this project keeps relearning: the API
settles it, not the docs, and not me. That sentence was written into D50 itself and then not
followed for another two turns.

**The finding is worth more than the correction costs.** F1 originally rested on documentation;
it now rests on eight paid transactions in a live account, each verified, all indistinguishable.
That is the strongest available answer to the obvious question — *why is your experiment
simulated?* — and it is now a reproducible artifact rather than an assertion:
`evidence/error-card-walk.md`, regenerated by `scripts/error_card_walk.py --verify`.

Worth noting what the classifier did with them: `payment_failed` is ambiguous, so the table
correctly refused, and the model inferred `TRANSIENT_ISSUER` at 0.75 from `source` and `step`
alone. That inference is almost certainly wrong for at least six of the eight — a disabled
card is not an issuer outage — but it is not the model's error. There is no information in the
payload to be right with. Which is the whole point of F1.

The `insufficient_fund` / `insufficient_funds` spelling question raised in D50 is now
unanswerable from this account, since no card returns either string. It stays open against
production data.

### Still open

- **No real Razorpay decline has reached the classifier as anything but ambiguous**, and on
  this account none can (D58). Production traffic would settle it; test mode cannot.
